...
M.FindCitationKey = function(str, cb)
    local mtchs = require("zotcite.get").match(str)
    ...
    for _, v in pairs(mtchs) do
        ...
        table.insert(M.sel_list, v.key)
    end
    pickers
        .new({}, {
            prompt_title = "Select Citation",
            finder = finders.new_table({
                results = references,
                entry_maker = function(entry)
                    local displayer = entry_display.create({
            ...
            attach_mappings = function(prompt_bufnr, map)
                map("i", "<CR>", function()
                    local selection = action_state.get_selected_entry()
                    actions.close(prompt_bufnr)
                    cb(nil, selection.index) -- Pass the selected index to the callback
                end)
                return true
            end,
        })
